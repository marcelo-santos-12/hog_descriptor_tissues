...
def get_hog(imgs, orientation, pixel_cell, cell_block):
    print('computando hogs...')
    return (hog(img, orientations=orientation, pixels_per_cell=pixel_cell, cells_per_block=cell_block, block_norm="L1") for img in imgs)
...
def create_data_hog(DATADIR, OUTDIR, orientation, pixel_cell, cell_block):
    ...
    for category in CATEGORIES:
        ...
        for img in tqdm.tqdm(imgs):
            ...
            hogs = hog(img_equ_adap, orientations=orientation, pixels_per_cell=pixel_cell, cells_per_block=cell_block, block_norm="L1")
            
            #np.savetxt(path_hog, hogs)
            save_file(hogs, path_hog)
